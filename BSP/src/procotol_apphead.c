...
static void Protocol_AppHead_ParamParse(PProtocol_Layer_ST layer,unsigned char* param,unsigned short datalen)
{
    ...
    for (index = 0;index<size;index++)
    {
        if (!memcmp(paramlist[index].param,param,strlen(paramlist[index].param)))
        {
            ...
            Protocol_AppHead_GetParamValue(param,datalen,value);
...
static void Protocol_AppHead_Recv(PProtocol_Layer_ST layer,unsigned char* data,unsigned short datalen)
{
    ...
    Protocol_AppHead_Parse(layer,++data,pbuffer-data-1);
    pbuffer++;
...
PProtocol_Layer_ST Protocol_AppHead_Initialize(void)
{
    if (!this)
    {
        ...
        this->init     = Protocol_AppHead_Init;
        this->pack     = Protocol_AppHead_Pack;
        this->unpack   = Protocol_AppHead_Unpack;
        this->getsize  = Protocol_AppHead_Getsize;
        this->puttag   = Protocol_AppHead_PutTag;
        this->clear    = Protocol_AppHead_Clear;
        this->setparam = Protocol_AppHead_SetParam;
        this->getparam = Protocol_AppHead_GetParam;
        this->send     = Protocol_AppHead_Send;
        this->recv     = Protocol_AppHead_Recv;
        this->getcrc   = Protocol_AppHead_GetCrc;
        this->setcrc   = Protocol_AppHead_SetCrc;
