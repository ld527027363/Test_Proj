...
uint8*  GET_SN_MASK    = strSystemParameter.LocalSubWay;					/*从DHCP服务器获取到的子网掩码*/
uint8*  GET_GW_IP      = strSystemParameter.LocalGateway;					/*从DHCP服务器获取到的网关地址*/
uint8*   GET_SIP        = strSystemParameter.LocalIp;				/*从DHCP服务器获取到的本机IP*/
...
{
    if(!W5500_Socket(SOCKET_DHCP,Sn_MR_UDP,DHCP_CLIENT_PORT,0x00))//初始化socket和端口
	{
			#ifdef DHCP_DEBUG	   
				printf("Fail to create the DHCPC_SOCK(%u)\r\n",SOCK_DHCP);
			#endif   
			return FALSE;														//socket初始化错误
	}
    else
    {
...
uint8 check_DHCP_state(SOCKET s) 
{
	...
	type = 0;
    if ((len = W5500_getSn_RX_RSR(s)) > 0)									/*接收到数据*/
    {
...
{
	uint8 dhcpret=0;
	dhcpret = check_DHCP_state(SOCKET_DHCP);             /*获取DHCP服务状态*/
	switch(dhcpret)
	{
